...
public class ChessBoardRenderer {
	...
	private Chess.ChessBoard board = new Chess.ChessBoardImplementation();
        private Chess.ChessPieceImplementation chess = null;
	...
	void drawPiece(Canvas canvas, Chess.ChessPiece piece, double minX, double minY, double width, double height, Bounds boardBounds) {
		GraphicsContext gc = canvas.getGraphicsContext2D();
                
                double widthx = boardBounds.getWidth() / 8;
		double heightx = boardBounds.getHeight() / 8;
                double minxbound=0;
                double minybound=0;
		...
		if (piece == movingPiece)
                {    
                       ...
                       //Cambiado de color al azul 
                        gc.setFill(Color.BLUE);
                        //recorremos todas las posibles posiciones de la pieza dada
                        for (PiecePosition piece3 : piece.getAvailablePositions(board))
                        {
                            //Cogemos la columna
                            int  c = piece3.getColumn();
                            //Cogemos la fila
                            int r = piece3.getRow();
                            //calculamos la posicion del recuadro en funcion de la posicion de la ficha
                            minxbound = boardBounds.getMinX() + c * widthx;
                            minybound = boardBounds.getMinY() + r * heightx;
                           
                            //Dibujamos un rectangualo con las posibles posiciones de la ficha
                            gc.fillRect(minxbound, minybound + heightx * 0.01, widthx, heightx );
                            minxbound=0;
                            minybound=0;
                            //
                            widthx =boardBounds.getWidth() / 8;
                            heightx=boardBounds.getHeight() / 8;
                           
                        }//Cambiamos al color gris que se iluminara con la ficha seleccionada
		...
		switch(piece.getType()) {
			...
			case BISHOP:
				...
				break;
			case KNIGHT:
                                drawTriangle(gc, minX, minY + height * 0.1, width, height * 0.5);
                                gc.fillRect(minX, minY + height * 0.6, width * 0.7 , height * 0.4);
	...
	void drawPiece(Canvas canvas, Bounds boardBounds, Chess.ChessPiece piece) {
		...
		drawPiece(canvas, piece, boardBounds.getMinX() + c * width, 
				boardBounds.getMinY() + r * height, width, height, boardBounds);
	}
	...
	void draw(Canvas canvas) {
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
                ...
                //aqui se dibujan las fichas en el tablero
		for (ChessPiece piece : board.getPieces())
			drawPiece(canvas, boardBounds, piece);

		gc.setFill(Color.BLACK);
                //aque se dibuja la informacion sobre las columnas y las celdas
		for (int i = 0; i < 8; i++) {
	...
	ChessPiece getPieceAt(Canvas canvas, double x, double y) {
        ...
        void setBoard(Chess.ChessBoard board) {
           this.board = board;
        }
        Chess.ChessBoard getBoard(Canvas canvas) {
           return board;
        }
